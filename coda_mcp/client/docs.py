...
class DocsClient(CodaRequestMixin):
    ...
    async def list_docs(self, query: DocsListQuery | None = None) -> DocsListResponse:
    ...
    async def create_doc(self, body: CreateDocBody) -> DocDetail:
    ...
    async def get_doc(self, doc_id: str) -> DocDetail:
        response = await self.http.get(self._doc(doc_id))
        response.raise_for_status()
        return validate_pydantic(DocDetail, response.json())

    async def patch_doc(self, doc_id: str, body: PatchDocBody) -> DocUpdateResult:
    ...
    async def delete_doc(self, doc_id: str) -> DocDeleteResult:
        response = await self.http.delete(self._doc(doc_id))
        response.raise_for_status()
        return validate_pydantic(DocDeleteResult, response.json())

    async def get_acl_metadata(self, doc_id: str) -> AclMetadataResponse:
        response = await self.http.get(self._doc(doc_id, "/acl/metadata"))
        response.raise_for_status()
    ...
    async def add_permission(self, doc_id: str, body: AddPermissionBody) -> AddPermissionResult:
        response = await self.http.post(
            self._doc(doc_id, "/acl/permissions"),
            json=body.model_dump(by_alias=True, exclude_none=True),
        )
    ...
    async def delete_permission(self, doc_id: str, permission_id: str) -> DeletePermissionResult:
    ...
    async def get_acl_settings(self, doc_id: str) -> AclSettingsResponse:
        response = await self.http.get(self._doc(doc_id, "/acl/settings"))
        response.raise_for_status()
    ...
    async def list_doc_categories(self) -> DocCategoriesResponse:
        response = await self.http.get(self.url("/categories"))
        response.raise_for_status()
        return validate_pydantic(DocCategoriesResponse, response.json())

    async def publish_doc(self, doc_id: str, body: PublishDocBody) -> PublishQueuedResponse:
    ...
    async def unpublish_doc(self, doc_id: str) -> UnpublishResult:
        response = await self.http.delete(self._doc(doc_id, "/publish"))
        response.raise_for_status()
